...
const UA = 'Nebula/1.0 (persoonlijke ambient-speler)';
const MB = 1024 * 1024;
...
export async function runFetch(opts = {}) {
  ...
  log(`Nebula: geluiden ophalen (${mode === 'all' ? 'alles' : 'snelle selectie'}${dry ? ', dry run' : ''}). Bibliotheek: ${lib.sounds.length} geluiden, ${(totalBytes / MB).toFixed(0)} MB. Limiet ${(maxBytes / 1024 / MB).toFixed(1)} GB.`);
