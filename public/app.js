...
(function () {
  ...
  const desktop = window.hushfallDesktop && window.hushfallDesktop.isDesktop;
  const engine = new window.HushfallEngine();
  const visuals = new window.HushfallVisuals($('#bg'));
  ...
  function synthSounds() {
    ...
    return window.HushfallSynth.list.map((g) => ({
      ...
      source: 'synth', sourceName: 'Hushfall', license: 'Hushfall\u2019s own sound, generated live in the app',
      author: 'Hushfall', seconds: null, bytes: 0, file: null, tags: ['own', g.desc],
    }));
  }
  ...
  function soundCard(s, { mix = false } = {}) {
    ...
    el.title = mix ? s.title : `${s.title}\n${s.synth ? (s.tags?.[1] || '') + ' · generated live by Hushfall, never repeats' : (s.sourceName || '') + ' · ' + (s.license || '')}`;
    return el;
  }
  function renderHome() {
    ...
    if (own.length) {
      const sec = document.createElement('section'); sec.className = 'row';
      sec.innerHTML = `<div class="row-head"><h3>Made by Hushfall <span class="count">${own.length} · generated live, never repeats</span></h3><button class="link">Show all</button></div>`;
  ...
  function renderMixer() {
    ...
    mk('alle', 'All'); mk('actief', 'Playing'); if (library.sounds.some((s) => s.synth)) mk('eigen', 'Made by Hushfall');
  ...
  function renderLibraryInfo() {
    ...
    $('#lib-summary').innerHTML = `<b>${own}</b> sounds made by Hushfall itself, always available and without any files.` +
    ...
    if (desktop) {
      ...
      $('#lib-stop').addEventListener('click', () => window.hushfallDesktop.stopFetch());
      $('#lib-open').addEventListener('click', () => window.hushfallDesktop.openFolder());
      $('#lib-reload').addEventListener('click', () => loadLibrary().then(() => toast('Library reloaded')));
      $('#lib-choose').addEventListener('click', async () => {
        const r = await window.hushfallDesktop.chooseFolder();
        if (r?.error) return toast(r.error);
        if (r?.canceled) return;
        await loadLibrary();
        toast(r.hasLib ? `Library loaded from ${r.dir}` : `Folder chosen: ${r.dir} (still empty, fetch some sounds)`);
      });
      $('#lib-default').addEventListener('click', async () => { await window.hushfallDesktop.resetFolder(); await loadLibrary(); toast('Default folder restored'); });
      window.hushfallDesktop.info().then((i) => { $('#lib-dir').textContent = `Folder: ${i.dir}`; $('#lib-default').hidden = !!i.isDefault; }).catch(() => {});
    } else {
    ...
    const sources = [
      ['Hushfall', null, 'Own sounds and music, generated live in the app'],
  ...
  async function runFetch(mode) {
    ...
    const unLog = window.hushfallDesktop.onLog((line) => { log.textContent += line + '\n'; if (log.textContent.length > 60000) log.textContent = log.textContent.slice(-40000); log.scrollTop = log.scrollHeight; });
    const unProg = window.hushfallDesktop.onProgress((p) => {
    ...
    try {
      const r = await window.hushfallDesktop.fetchLibrary(mode);
  ...
  function updatePlayer() {
    ...
    if ('mediaSession' in navigator) {
      try { navigator.mediaSession.metadata = new MediaMetadata({ title, artist: sub, album: 'Hushfall' }); navigator.mediaSession.playbackState = engine.playing ? 'playing' : 'paused'; } catch {}
    }
  }
  ...
  if (desktop && window.hushfallDesktop.sonos) sonosBtn.hidden = false;
  ...
  function renderSonos() {
    ...
    const rooms = sonos.groups.map((g) => {
      ...
      return `<div class="sonos-room${on ? ' on' : ''}" data-host="${esc(g.host)}">
        <div><div class="n">${esc(g.name)}</div><div class="s">${esc(g.model || 'Sonos')}${on ? ' · playing Hushfall' : ''}</div></div>
    ...
    $$('.sonos-room', sonosPop).forEach((el) => {
      ...
      if (vol) vol.addEventListener('input', () => { vol.nextElementSibling.textContent = vol.value; const g = sonos.groups.find((x) => x.host === host); if (g) g.volume = Number(vol.value); window.hushfallDesktop.sonos.volume(host, Number(vol.value)); });
    });
    updateSonosStatus();
  }
